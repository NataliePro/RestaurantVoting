...
import ru.proshkina.restaurantvoting.util.ValidationUtil;
import ru.proshkina.restaurantvoting.util.exception.*;

import javax.servlet.http.HttpServletRequest;
import java.util.Map;
import java.util.Optional;

import static ru.proshkina.restaurantvoting.util.exception.ErrorType.*;
...
public class ExceptionInfoHandler {
    ...
    public static final String EXCEPTION_DUPLICATE_DATE_USER = "exception.vote.duplicateDateUser";

    private static final Map<String, String> CONSTRAINS_I18N_MAP = Map.of(
            "users_unique_email_idx", EXCEPTION_DUPLICATE_EMAIL,
            "votes_unique_date_user_id_idx", EXCEPTION_DUPLICATE_DATE_USER);

    @Autowired
    private MessageUtil messageUtil;

    @ResponseStatus(value = HttpStatus.UNPROCESSABLE_ENTITY)  // 422
    @ExceptionHandler(VotingTimeIsOutException.class)
    public ResponseEntity<ErrorInfo> votingTimeIsOutError(HttpServletRequest req, VotingTimeIsOutException appEx) {
